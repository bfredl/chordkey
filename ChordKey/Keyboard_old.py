...
##################
    ...
    def key_down(self, key, view = None,
        ...
        if key.sensitive:
            ...
            if action == KeyCommon.FIXED_DOUBLE_STROKE_ACTION:
                self.key_up(key, view, button, event_type, False, True)

    def key_up(self, key, view = None, button = 1,
               event_type = EventType.CLICK, cancel_send_key = False, immediate_up=False):
        """ Release one of Onboard's key representations. """
        update = False
        if key.action == KeyCommon.FIXED_DOUBLE_STROKE_ACTION and not immediate_up:
            return
    ...
    def send_key_press(self, key, view, button, event_type):
        ...
        elif key_type == KeyCommon.SEQUENCE_TYPE:
            for mod in key.seq_modifiers:
